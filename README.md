[![Open in Codespaces](https://classroom.github.com/assets/launch-codespace-2972f46106e565e64193e422d61a12cf1da4916b45550586e14ef0a7c637dd04.svg)](https://classroom.github.com/open-in-codespaces?assignment_repo_id=20028226)
...
```
 In the main() method, the program creates two servers running simultaneously on ports 8080 and 8081, each in its own separate thread. These servers wait to receive requests from users on their respective ports. The program then prompts the user to press a key to stop the servers. Once a key is pressed, the program stops both servers and terminates gracefully.
...
Create new MockWebServer instances with different port numbers and start each in a separate thread.

-Run multiple servers simultaneously:
Start multiple server instances on different ports using separate threads to run them at the same time.

-Change HTML content:
Modify the response HTML string to include tables, images, or more text using standard HTML tags.

-Benefits of multiple servers:
Allows serving different services on different ports, improves flexibility, and helps with load distribution and testing.
```
